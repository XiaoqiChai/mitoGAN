...
candidate_folder = 'CycleGAN/candidateA'

os.mkdir('CycleGAN/test_predicted_cgan')
os.mkdir('CycleGAN/candidate_predicted')
...
for idx in range(len(img_list_test)):
    ...
    cv2.imwrite('CycleGAN/test_predicted_cgan/'+test_name,img_test_predicted)
...
for idx in range(len(img_list_candidate)):
    ...
    cv2.imwrite('CycleGAN/candidate_predicted/'+candidate_name,img_candi_predicted)
